...
function Map({
  ...
  const [presentCountModal, dismissCount] = useIonModal(Count, {
    onDismiss: (data: string, role: string) => dismissCount(data, role),
  });
    ...
    <IonCard>
      <IonCardHeader className="ion-padding-horizontal">
        ...
        <IonCardSubtitle className="ion-no-padding">
          ...
          <IonButton onClick={() => presentCountModal()} fill="clear">
            <IonIcon
              className="ion-no-padding"
              color="primary"
              slot="icon-only"
